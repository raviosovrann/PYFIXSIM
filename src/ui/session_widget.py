...
class SessionListWidget(QGroupBox):
	...
	start_requested = Signal()  # emitted when the user chooses Start for the selected session(s)
	stop_requested = Signal()  # emitted when the user chooses Stop for the selected session(s)
	restart_requested = Signal()  # emitted when the user chooses Restart for the selected session(s)
	show_session_messages_requested = Signal()  # emitted when session messages are requested
	...
	def __init__(self, parent: QWidget | None = None) -> None:
		...
		self.set_sessions(build_placeholder_sessions())
	...
	def _create_context_actions(self) -> None:
		...
		self._show_messages_action.triggered.connect(self._on_show_session_messages_requested)
