...
class SendMessageTab(QWidget):
	...
	send_selected_requested = Signal()  # emitted when the user requests sending selected messages
	send_current_and_next_requested = Signal()  # emitted when the user sends the current block and advances
	reset_requested = Signal()  # emitted when the user resets send progression
	record_test_scenario_requested = Signal()  # emitted when the user starts recording a test scenario
	...
	insert_soh_requested = Signal()  # emitted when the user inserts a SOH delimiter into the editor
	...
	search_text_changed = Signal(str)  # emitted when the raw-message search text changes
	...
	def __init__(self, parent: QWidget | None = None) -> None:
		...
		self._message_editor.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
		...
		self.set_message_text(
			"8=FIX.4.4|35=D|49=CLIENT_A|56=BROKER_A|11=ORDER_1|55=ABCDEF|54=1|38=100|40=2|44=25.50|\n"
			"8=FIX.4.4|35=F|49=CLIENT_A|56=BROKER_A|11=ORDER_2|41=ORDER_1|55=ABCDEF|54=1|38=100|"
		)
	...
	def _wire_signals(self) -> None:
		...
		self._send_current_next_button.clicked.connect(self.send_current_and_next_requested)
		self._reset_button.clicked.connect(self.reset_requested)
		self._record_scenario_button.clicked.connect(self.record_test_scenario_requested)
		...
		self._message_editor.customContextMenuRequested.connect(self._on_editor_context_menu_requested)
