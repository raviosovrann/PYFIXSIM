...
description: "Use when working on FIX engine logic, session management, connectivity, message routing, or backend configuration in src/engine/, src/messages/, or src/config/. Handles session lifecycle, logon/logout sequences, heartbeat management, order flow, execution reports, configuration persistence, and Python code quality review. Expert in Python and FIX protocol. MUST BE USED for all backend Python changes in this project."
